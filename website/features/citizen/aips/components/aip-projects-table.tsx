...
const PAGE_SIZE = 10;
const DETECTED_COLOR = "#C28A3D";
const REVIEWED_COLOR = "#4F8B94";
const TEXT_COLOR = "#1F2937";
const NO_ISSUE_COLOR = "#FFFFFF";
  ...
  return (
    <Card data-testid="citizen-aip-projects-table" className="border-slate-200">
      <CardHeader className="px-4 pb-3 pt-4 sm:px-6 sm:pb-4 sm:pt-6">
        <CardTitle className="text-xl sm:text-2xl" style={{ color: TEXT_COLOR }}>{aip.title} Details</CardTitle>
      </CardHeader>
      ...
      <CardContent className="space-y-4 px-4 pb-4 sm:px-6 sm:pb-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <div className="w-full md:w-[280px]">
            <label className="text-xs" style={{ color: TEXT_COLOR }}>Search Projects</label>
        ...
        <div className="inline-flex items-center gap-2 text-xs" style={{ color: TEXT_COLOR }}>
          <CircleHelp className="h-3.5 w-3.5" />
          Tip: Select a row to view the project&apos;s full details.
        </div>

        {unresolvedAiCount > 0 ? (
          <div className="rounded-md border px-3 py-2 text-xs" style={{ borderColor: DETECTED_COLOR, backgroundColor: "#FAF3E8", color: TEXT_COLOR }}>
          ...
          <Table className="min-w-[640px]">
            <TableHeader>
              <TableRow className="bg-slate-50 hover:bg-slate-50">
                ...
                <TableHead className="text-right text-xs font-semibold" style={{ color: TEXT_COLOR }}>Total Amount</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {visibleRows.map((row) => (
                ...
                >
                  <TableCell className="text-sm break-words" style={{ color: TEXT_COLOR }}>{row.projectRefCode}</TableCell>
                  <TableCell className="text-sm break-words" style={{ color: TEXT_COLOR }}>{row.programDescription}</TableCell>
                  <TableCell className="text-right text-sm whitespace-nowrap" style={{ color: TEXT_COLOR }}>{formatCurrency(row.totalAmount)}</TableCell>
                </TableRow>
              ))}
        ...
        <div className="flex flex-wrap gap-3 pt-1 text-[11px] sm:justify-end sm:gap-5 sm:pt-2 sm:text-xs" style={{ color: TEXT_COLOR }}>
          <div className="inline-flex items-center gap-2">
            <span className="inline-block h-3.5 w-3.5 rounded-sm" style={{ backgroundColor: DETECTED_COLOR }} aria-hidden="true" />
            AI-flagged with no LGU feedback note
          </div>
          <div className="inline-flex items-center gap-2">
            <span className="inline-block h-3.5 w-3.5 rounded-sm" style={{ backgroundColor: REVIEWED_COLOR }} aria-hidden="true" />
            Has LGU feedback note
          </div>
          <div className="inline-flex items-center gap-2">
            <span className="inline-block h-3.5 w-3.5 rounded-sm border" style={{ borderColor: TEXT_COLOR, backgroundColor: NO_ISSUE_COLOR }} aria-hidden="true" />
            No issues detected
          </div>
        </div>
      </CardContent>
